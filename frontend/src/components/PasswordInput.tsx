...
// @ts-expect-error Needed to reuse props
...
const PasswordInput : React.FC<InputProps> = (props) => {
    ...
    return (
        <Input label={props.label} variant={props.variant} type={isVisible ? "text" : "password"} endContent={
            <button className={"w-[26px] h-[26px] self-center"} onClick={toggleVisibility}>
                {isVisible ? (
                    <EyeSlashFilledIcon />
                ) : (
                    <EyeFilledIcon />
